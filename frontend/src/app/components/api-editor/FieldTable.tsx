          ...
          {fields.length > 0 ? (
              ...
              <tr key={field.id} className="group hover:bg-gray-50/50 transition-colors">
                <td className="px-4 py-2">
                  <input
                    ...
                    disabled={readOnly}
                    className="w-full bg-transparent border-0 focus:ring-0 text-sm font-medium text-gray-900 placeholder:text-gray-300 disabled:opacity-80"
                ...
                <td className="px-4 py-2">
                  <select
                    ...
                    disabled={readOnly}
                    className="w-full bg-transparent border-0 focus:ring-0 text-sm text-gray-600 appearance-none cursor-pointer disabled:opacity-80"
                  >
                    {fieldTypes.map((t) => (
                ...
                <td className="px-4 py-2">
                  <input
                    ...
                    disabled={readOnly}
                    className="w-full bg-transparent border-0 focus:ring-0 text-sm text-gray-500 placeholder:text-gray-300 disabled:opacity-80"
                ...
                <td className="px-4 py-2 text-center">
                  <input
                    ...
                    disabled={readOnly}
                    className="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500/20 disabled:opacity-80"
                  />
                </td>
            ...
            <tr>
              <td colSpan={5} className="px-4 py-10 text-center text-gray-400 text-xs font-medium">
                등록된 필드가 없습니다. {readOnly ? '' : '필드를 직접 추가하거나 JSON을 가져와보세요.'}
              </td>
            </tr>
          )}
          {!readOnly && (
